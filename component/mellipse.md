...
将`m-ellipse-tool`放入`m-toolbar`插槽中即可
...
将`m-ellipse-tool`放入`m-toolbar`插槽中,设置`size`和`predefine`属性即可
...
<template>
  <div style="height:400px;">
      ...
      <template v-slot:footer>
        <m-toolbar>
          <m-ellipse-tool :size="size" :predefine="predefine"></m-ellipse-tool>
          <m-undo-tool></m-undo-tool>
          <m-file-tool></m-file-tool>
        </m-toolbar>
      </template>
